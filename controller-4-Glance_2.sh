...
keystone endpoint-create --service-id=$(keystone service-list | awk '/ image / {print $2}') --publicurl=http://$CON_EXT_IP:9292 --internalurl=http://controller:9292 --adminurl=http://controller:9292
